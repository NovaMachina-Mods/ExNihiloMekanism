...
public class ExNihiloMekanismRecipeGenerator extends RecipeProvider {
  ...
  private void registerSieve(Consumer<FinishedRecipe> consumer) {
    SiftingRecipeBuilder.sifting(
            Ingredient.of(Blocks.GRAVEL), ExNihiloMekanismItems.OSMIUM_PIECES.get())
  ...
  private ResourceLocation siftingLoc(String id) {
    return new ResourceLocation(
        ModIds.EX_NIHILO_MEKANISM, "sifting/" + RecipeProviderUtilities.prependRecipePrefix(id));
  }

  private ResourceLocation heatLoc(String id) {
    return new ResourceLocation(
        ModIds.EX_NIHILO_MEKANISM, "heat/" + RecipeProviderUtilities.prependRecipePrefix(id));
  }
}
